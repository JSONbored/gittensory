...
export function planAgentMaintenanceActions(input: AgentActionPlanInput): PlannedAgentAction[] {
  ...
  // 2) review — APPROVE a review-good PR only when it is not guarded. A guarded PR falls through to the owner's
  // manual safety review. A not-good OWNER/automation PR is HELD with request-changes (the maintainer sees what
  // to fix). A not-good CONTRIBUTOR PR is CLOSED below and the close comment carries the reasoning — no redundant
  // request-changes.
