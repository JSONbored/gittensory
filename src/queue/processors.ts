...
import { buildUnifiedReviewDiff, addedLineCount } from "../review/review-diff";
...
export async function resolveAutoReviewSkipForPullRequest(
  ...
  const skipReason = resolvePullRequestAutoReviewSkipReason({
    ...
    changedFileCount: args.changedFileCount ?? args.changedPaths?.length ?? 0,
...
async function maybePublishPrPublicSurface(
  ...
  try {
    ...
    const autoReviewAddedLines = autoReviewFiles.reduce(
      (sum, file) => sum + addedLineCount(typeof file.payload?.patch === "string" ? file.payload.patch : undefined),
      0,
    );
    ({
      ...
      addedLineCount: autoReviewAddedLines,
      changedFileCount: autoReviewChangedPaths.length,
    }));
