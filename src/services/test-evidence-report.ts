...
export type TestEvidenceReport = {
  ...
  /** Number of changed paths supplied. */
  changedPathCount: number;
  /** How many of the changed paths are themselves test files. */
...
export function buildTestEvidenceReport(changedPaths: string[], testFiles?: string[]): TestEvidenceReport {
  const coverageBand = classifyTestCoverage(changedPaths);
  const testPathCount = changedPaths.filter(isTestPath).length;
  const hasCodeChanges = changedPaths.some(isCodeFile);
  // Evidence if the caller flagged explicit test paths (`tests`) OR any changed path is itself a test file
  // (`testFiles` is filtered through isTestPath inside the classifier). Mirrors buildBoundaryTestGenerationFinding.
  const hasTestEvidence = hasLocalTestEvidence({ tests: testFiles, testFiles: changedPaths });
  return {
    coverageBand,
    ...
    summary: `Test-evidence check: ${coverageBand} coverage across ${changedPaths.length} changed path(s), ${testPathCount} test file(s).`,
  };
}
