...
import { currentOtelTraceIds, withOtelSpan } from "./otel";
...
/** The shared review-pipeline span wrapper: open ONE boundary that feeds BOTH tracers — an OpenTelemetry span and a
 *  Sentry span — so instrumentation is consistent and the same stage shows up in whichever backend is enabled. Each
 *  side independently no-ops when its backend is off, so this reduces to `fn()` when neither is configured. (#1734) */
