...
// not guaranteed to exist yet when typecheck/test:coverage run in CI). Issue-side triage below
// (buildIssueSlopAssessment and friends) was NOT extracted — it is not needed by the miner's self-review path
// and stays here unchanged, reusing this shim's re-exported SlopAssessment/SlopBand/clamp/slopBandFor so the
// two sides continue to share identical band math.
export {
...
import { clamp, slopBandFor, type SlopAssessment, type SlopBand } from "../../packages/gittensory-engine/src/signals/slop";
