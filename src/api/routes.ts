...
const repositorySettingsSchema = z.object({
  ...
  slopGateMode: z.enum(["off", "advisory", "block"]).default("advisory"),
  slopGateMaxRisk: z.number().int().min(0).max(100).nullable().optional(),
  ...
  app.post("/v1/internal/repos/:owner/:repo/settings", async (c) => {
    ...
    return c.json(
      await upsertRepositorySettings(c.env, {
        ...
        slopGateMode: parsed.data.slopGateMode,
        slopGateMaxRisk: parsed.data.slopGateMaxRisk,
