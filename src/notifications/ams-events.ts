...
//
// Payload/dedupKey shape is mirrored in packages/loopover-miner/lib/ams-notifications.ts (miner cannot import
// src/); keep the two builders in lockstep when changing eventType strings or dedupKey layout.
...
export function buildAmsPrOutcomeEvent(input: {
  ...
  return {
    ...
    // decision sits immediately after the eventType so buildAmsPrOutcomeNotification can read it without
    // scanning an opaque middle segment (ams_pr_outcome:{merged|closed}:{repo}#{n}:{closedAt}).
...
export function normalizeAmsNotificationEventInput(
  ...
  return {
    ...
    // Force actor to the authenticated recipient — never trust a client-supplied actorLogin.
    actorLogin: login,
