 ...
 * gate trio that `recommendedAdvisoryActivationSettings` enables in advisory mode; blocking ramps those same
 * fields to `block` via the existing PUT /settings merge path (maintainer-settings.tsx).
 */

export type GateMode = "off" | "advisory" | "block";

/** The three deterministic sub-gates flipped together during the CodeRabbit-style onboarding ramp. */
...
/** Whether the Gittensory gate is actively publishing (parity with buildMaintainerActivationPreview). */
export function isGateRampActive(settings: GateRampSettingsSlice): boolean {
  return settings.reviewCheckMode !== undefined && settings.reviewCheckMode !== "disabled";
}
