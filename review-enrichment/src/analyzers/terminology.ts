...
// are the exact lowercased TOKENS the tokenizer produces — inflections are listed explicitly rather than matched
// by a fuzzy stem, so there is no accidental over-match. `master`/`slave` are intentionally exact singular/plural
// only (no `mastered`/`slavery`, which are unrelated words).
 ...
 *  `master-branch` all separate), then split each run on camelCase boundaries. Pure. */
