...
const KUBE_INSECURE_TLS_RE = /--insecure-skip-tls-verify\b/;
...
export function scanPatchForIacMisconfig(
  ...
  for (const line of patchLines(patch)) {
    ...
    if (
      KUBE_INSECURE_TLS_RE.test(body) &&
      pushFinding(findings, seen, path, newLine, "kube-insecure-tls", maxFindings)
    ) {
      return findings;
    }
