...
run_node_build() {
  ...
  docker run --rm \
    ...
    -w /work \
    "$NODE_IMAGE" \
    sh -lc 'npm ci --ignore-scripts && npm --workspace @jsonbored/loopover-engine run build && node scripts/build-selfhost.mjs --all && node scripts/validate-selfhost-sourcemap.mjs'
}
