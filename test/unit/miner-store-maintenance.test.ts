...
describe("resolveLedgerRetentionPolicy (#4834)", () => {
  ...
  it("ignores zero, negative, blank, and non-numeric values (treated as unset)", () => {
    ...
    // floors a fractional day count
    expect(resolveLedgerRetentionPolicy({ [LEDGER_RETENTION_DAYS_ENV]: "2.9" })).toEqual({ maxAgeMs: 2 * 86_400_000 });
  });
});
