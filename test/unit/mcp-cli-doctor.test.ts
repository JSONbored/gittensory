...
describe("gittensory-mcp CLI — doctor", () => {
  ...
  it("reports a stale global install with an exact upgrade command and npx fallback", async () => {
    ...
    expect(payload.package).toMatchObject({
      ...
      upgradeCommand: "npm install -g @jsonbored/loopover-mcp@latest",
    });
    expect(payload.package.npxFallback).toContain("npx @jsonbored/loopover-mcp@latest");
  });
  ...
  it("flags a stale install in doctor with upgrade remediation", async () => {
    ...
    expect(version?.remediation).toContain("npm install -g @jsonbored/loopover-mcp@latest");
    expect(version?.remediation).toContain("npx @jsonbored/loopover-mcp@latest");
  });
  ...
  it("flags API compatibility mismatches with upgrade guidance", async () => {
    ...
    expect(doctor.checks).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          status: "fail",
          remediation: "npm install -g @jsonbored/loopover-mcp@latest",
        }),
      ]),
    );
    ...
    expect(doctor.nextCommand).toMatchObject({
      command: "npm install -g @jsonbored/loopover-mcp@latest",
  ...
  it("does not print configured tokens or local absolute paths in status or doctor output", async () => {
    ...
    expect(statusOutput).toContain("npm install -g @jsonbored/loopover-mcp@latest");
  });
