...
// A token-shaped value planted in the RAW backing data for the redacting tools (audit-feed, governor). Because
// each tool projects to a metadata-only shape, this must never reach the response — so the `valid` assertions are
// real regression guards (they'd fail if a projection ever widened to spread the raw row), not vacuous no-ops on
// an empty fixture.
const PLANTED_TOKEN = "ghp_0123456789abcdefABCD";
...
function seededGovernorLedger(): ReturnType<typeof initGovernorLedger> {
  ...
  ledger.appendGovernorEvent({
    ...
    payload: { reputation: 0.2, self_plagiarism: true, budget: { remaining: 0 }, token: PLANTED_TOKEN },
  });
  return ledger;
}
        ...
        readEvents: () => [
          {
            ...
            payload: { outcome: "merged", actor: "miner", detail: "ok", token: PLANTED_TOKEN, payload_json: "raw" },
