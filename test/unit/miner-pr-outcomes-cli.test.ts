...
describe("parsePrOutcomesArgs (#7658)", () => {
  ...
  it("distinguishes an empty value from a dash-prefixed value for --login and --limit", () => {
    // `--login` with a following flag exercises the value.startsWith("-") arm; `--login` at end exercises !value.
    expect(parsePrOutcomesArgs(["--login", "--limit"])).toEqual({ error: expect.stringContaining("--login") });
    expect(parsePrOutcomesArgs(["--login", "octocat", "--limit", "--json"])).toEqual({ error: expect.stringContaining("--login") });
    // A dash-prefixed --limit value is rejected by the startsWith("-") guard BEFORE the numeric range check.
    expect(parsePrOutcomesArgs(["--login", "octocat", "--limit", "-5"])).toEqual({ error: expect.stringContaining("--login") });
    // --json before --login still requires --login (login stays null through the loop).
    expect(parsePrOutcomesArgs(["--json"])).toEqual({ error: expect.stringContaining("--login") });
  });
});
...
describe("runPrOutcomes (#7658)", () => {
  ...
  it("renders each outcome field independently: string repo/recordedAt + numeric pullNumber vs their fallbacks", async () => {
    captureConsole();
    // A fully-typed outcome (all string/number branches take the present side) alongside one whose fields are all
    // absent (every fallback side) — so both arms of each of renderOutcome's three conditionals are exercised.
    const mixed = {
      login: "octocat",
      count: 2,
      summary: "s",
      outcomes: [
        { repoFullName: "acme/w", pullNumber: 7, outcome: "merged", attribution: "o", deeplink: "d", recordedAt: "2026-07-03T00:00:00Z" },
        { outcome: "merged" },
      ],
    } as unknown as ContributorPrOutcomes;
    const code = await runPrOutcomes(["--login", "octocat"], { fetchContributorPrOutcomes: async () => mixed });
    expect(code).toBe(0);
    const out = logs.join("\n");
    expect(out).toContain("acme/w #7  merged  2026-07-03T00:00:00Z");
