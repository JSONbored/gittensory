...
const otelMocks = vi.hoisted(() => ({
  ...
  // withOtelSpan no-ops to its callback here (OTEL off) — so withReviewSpan exercises only the Sentry side.
  withOtelSpan: vi.fn(<T>(_name: string, _attrs: unknown, fn: () => T): T => fn()),
}));
vi.mock("@sentry/node", () => ({
  init: mocks.init,
...
vi.mock("../../src/selfhost/otel", () => ({
  ...
  withOtelSpan: otelMocks.withOtelSpan,
}));
...
describe("tracing is a complete no-op unless sampling is configured > 0 (#1734)", () => {
  it("with Sentry off, the span helpers run fn but start NO span and report tracing disabled", async () => {
    expect(sentryTracingEnabled()).toBe(false);
    expect(await withSentrySpan("s", { a: 1 }, async () => "r")).toBe("r");
    expect(await withReviewSpan("s", { a: 1 }, async () => "r2")).toBe("r2");
  ...
  it("with the DSN set but sample rate 0 (default), tracing stays off and starts no span", async () => {
    ...
    await withReviewSpan("s", undefined, async () => "r");
    expect(mocks.startSpan).not.toHaveBeenCalled();
    // withReviewSpan still runs the OTEL side (its own no-op here), so the boundary is preserved.
    expect(otelMocks.withOtelSpan).toHaveBeenCalledTimes(1);
  });
});
...
describe("tracing emits spans when sampling is enabled (#1734)", () => {
  ...
  it("withReviewSpan drives BOTH the OTEL and the Sentry span for one boundary", async () => {
  ...
  it("propagates an error thrown by fn so the span is recorded as failed", async () => {
    await expect(
