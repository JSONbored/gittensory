...
describe("gittensory-miner backup/restore (#4872)", () => {
  it("REGRESSION (gate-caught): covers every one of the sixteen local stores DEPLOYMENT.md documents, not just the subset doctor/migrate happen to track -- an incomplete list here means a silent, undetectable data-loss gap for anyone relying on `backup`", () => {
    const deploymentDoc = readFileSync(
      join(process.cwd(), "packages", "gittensory-miner", "DEPLOYMENT.md"),
      "utf8",
    );
    const documentedStoreFiles = [...deploymentDoc.matchAll(/^\s{5}([a-z][a-z0-9-]*)\.sqlite3\s/gm)].map(
      (match) => match[1],
    );
    expect(documentedStoreFiles.length).toBeGreaterThanOrEqual(16); // self-defense floor: fail loudly if this
    // regex ever stops matching DEPLOYMENT.md's real list, rather than silently passing with zero entries.
    expect(new Set(STORE_NAMES)).toEqual(new Set(documentedStoreFiles));
  });

  it("covers all sixteen local stores, in a stable order, and skips every one when nothing has been created yet (never creating them as a side effect)", async () => {
