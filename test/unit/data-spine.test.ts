...
describe("data spine repositories", () => {
  ...
  it("listOpenItemsByAuthorAcrossInstall (#2562) lists one author's open PRs + open issues across every repo THIS INSTALLATION tracks, excludes closed items/other authors/other installations, and matches case-insensitively", async () => {
    const env = createTestEnv();
    ...
    expect(await listOpenItemsByAuthorAcrossInstall(env, 456, "farmer99")).toEqual([{ repoFullName: "other-owner/other-install-repo", number: 6, kind: "pull_request" }]);
    // Case-insensitive: a differently-cased login still matches the same rows.
    expect(await listOpenItemsByAuthorAcrossInstall(env, 123, "FARMER99")).toHaveLength(3);
    // An author with no open items anywhere lists nothing.
    expect(await listOpenItemsByAuthorAcrossInstall(env, 123, "nobody")).toEqual([]);
