...
describe("planReplaySnapshotPath (#3010) — pure, deterministic", () => {
  ...
  it("rejects whitespace-only commitSha (#7796)", () => {
...
describe("openReplaySnapshotStore (#3010) — round-trip persistence", () => {
  ...
  it("getSnapshot rejects a non-hex commitSha (#7796)", () => {
    const store = tempStore();
    expect(() => store.getSnapshot("acme/widgets", "../escape")).toThrow("invalid_commit_sha");
  });
});
