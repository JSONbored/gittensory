...
describe("resolveReviewPathInstructions (#review-path-instructions)", () => {
  ...
  it("parses review.max_findings, round-trips, and warns on invalid values (#2049)", () => {
    const capped = parseFocusManifest({ review: { max_findings: { blockers: 3, nits: 5 } } });
    expect(capped.review.maxFindings).toEqual({ blockers: 3, nits: 5 });
    expect(capped.review.present).toBe(true);
    expect(parseFocusManifest({ review: reviewConfigToJson(capped.review) }).review.maxFindings).toEqual({
      blockers: 3,
      nits: 5,
    });
    expect(parseFocusManifest({ review: {} }).review.maxFindings).toEqual({ ...EMPTY_MAX_FINDINGS_CONFIG });
    const partial = parseFocusManifest({ review: { max_findings: { nits: 2 } } });
    expect(partial.review.maxFindings).toEqual({ blockers: null, nits: 2 });
    const badShape = parseFocusManifest({ review: { max_findings: "five" } });
    expect(badShape.review.maxFindings).toEqual({ ...EMPTY_MAX_FINDINGS_CONFIG });
    expect(badShape.warnings.some((w) => /review\.max_findings.*must be an object/.test(w))).toBe(true);
    const badValue = parseFocusManifest({ review: { max_findings: { blockers: -1, nits: 1.5 } } });
    expect(badValue.review.maxFindings).toEqual({ blockers: null, nits: null });
    expect(badValue.warnings.some((w) => /review\.max_findings\.blockers/.test(w))).toBe(true);
    expect(badValue.warnings.some((w) => /review\.max_findings\.nits/.test(w))).toBe(true);
    expect(resolveReviewPromptOverrides(capped).maxFindings).toEqual({ blockers: 3, nits: 5 });
    expect(resolveReviewPromptOverrides(parseFocusManifest({})).maxFindings).toEqual({ ...EMPTY_MAX_FINDINGS_CONFIG });
  });
});
