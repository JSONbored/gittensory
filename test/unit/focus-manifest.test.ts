...
describe("resolveReviewPathInstructions (#review-path-instructions)", () => {
  ...
  it("resolveReviewPromptOverrides: non-null manifest passes the config through; null manifest → defaults", () => {
    ...
    // A null manifest (load failure) yields the byte-identical defaults; suggestions + changed-files summary +
    // effort score + impact map + culture profile + finding categories + security focus default OFF (strict
    // false). inlineComments (#4099) preserves the manifest's tri-state instead — a null manifest is "unset", not
    // an explicit false, so it resolves to null (consumed by shouldRequestInlineFindings's own precedence).
    expect(resolveReviewPromptOverrides(null)).toEqual({ profile: null, tone: null, securityFocus: false, inlineComments: null, suggestions: false, changedFilesSummary: false, effortScore: false, impactMap: false, cultureProfile: false, findingCategories: false, inlineCommentsPerCategory: null, minFindingSeverity: null, maxFindings: { blockers: null, nits: null }, commentVerbosity: null, pathInstructions: [], instructions: null, excludePaths: [], pathFilters: [], selfHostAiModel: { ...EMPTY_SELF_HOST_AI_MODEL_CONFIG } });
    // inlineComments (#4099): explicit true/false pass through as-is; absent resolves to null (unset), NOT false —
    // distinct from every other flag on this object, which collapses absent to strict false.
    expect(resolveReviewPromptOverrides(parseFocusManifest({ review: { inline_comments: false } })).inlineComments).toBe(false);
    expect(resolveReviewPromptOverrides(parseFocusManifest({ review: { inline_comments: true } })).inlineComments).toBe(true);
    expect(resolveReviewPromptOverrides(parseFocusManifest({ review: { profile: "chill" } })).inlineComments).toBeNull();
