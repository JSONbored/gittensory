...
describe("resolveReviewPathInstructions (#review-path-instructions)", () => {
  ...
  it("resolveReviewPromptOverrides: non-null manifest passes the config through; null manifest → defaults", () => {
    ...
    expect(resolveReviewPromptOverrides(manifest)).toEqual({ profile: "chill", tone: null, securityFocus: true, inlineComments: true, suggestions: true, changedFilesSummary: true, effortScore: true, impactMap: true, cultureProfile: true, autoMergeSummary: false, findingCategories: true, minFindingSeverity: null, maxFindings: { blockers: null, nits: null }, commentVerbosity: "detailed", pathInstructions: [{ path: "src/**", instructions: "be strict" }], instructions: "Follow our async-error conventions.", excludePaths: ["**/*.lock"], pathFilters: ["src/**", "!src/generated/**"], selfHostAiModel: { ...EMPTY_SELF_HOST_AI_MODEL_CONFIG } });
    // A null manifest (load failure) yields the byte-identical defaults; inline comments + suggestions + changed-files summary + effort score + impact map + culture profile + auto-merge summary + finding categories + security focus default OFF.
    expect(resolveReviewPromptOverrides(null)).toEqual({ profile: null, tone: null, securityFocus: false, inlineComments: false, suggestions: false, changedFilesSummary: false, effortScore: false, impactMap: false, cultureProfile: false, autoMergeSummary: false, findingCategories: false, minFindingSeverity: null, maxFindings: { blockers: null, nits: null }, commentVerbosity: null, pathInstructions: [], instructions: null, excludePaths: [], pathFilters: [], selfHostAiModel: { ...EMPTY_SELF_HOST_AI_MODEL_CONFIG } });
  ...
  it("parses review.auto_merge_summary (default OFF), marks present, round-trips, and warns on a non-boolean (#2051)", () => {
    expect(parseFocusManifest({ review: { auto_merge_summary: true } }).review.autoMergeSummary).toBe(true);
    ...
    const off = parseFocusManifest({ review: { auto_merge_summary: false } });
    expect(off.review.autoMergeSummary).toBe(false);
    expect(off.review.present).toBe(true);
    expect(parseFocusManifest({ review: {} }).review.autoMergeSummary).toBeNull();
    const bad = parseFocusManifest({ review: { auto_merge_summary: "yes" } });
    expect(bad.review.autoMergeSummary).toBeNull();
    expect(bad.warnings.some((w) => /review\.auto_merge_summary.*must be a boolean/.test(w))).toBe(true);
    expect(resolveReviewPromptOverrides(on).autoMergeSummary).toBe(true);
  });
