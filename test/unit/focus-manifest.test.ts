...
describe("public-safe invariant", () => {
  ...
  it("never emits public next steps that contain forbidden language for arbitrary manifests", () => {
    const arbManifest = fc.record({
      wantedPaths: fc.array(fc.string(), { maxLength: 4 }),
      blockedPaths: fc.array(fc.string(), { maxLength: 4 }),
      preferredLabels: fc.array(fc.string(), { maxLength: 4 }),
      linkedIssuePolicy: fc.constantFrom("required", "preferred", "optional"),
      issueDiscoveryPolicy: fc.constantFrom("encouraged", "neutral", "discouraged"),
      maintainerNotes: fc.array(fc.string(), { maxLength: 4 }),
      publicNotes: fc.array(fc.string(), { maxLength: 4 }),
    });
    fc.assert(
      fc.property(arbManifest, fc.array(fc.string(), { maxLength: 6 }), (raw, changedPaths) => {
        const manifest: FocusManifest = parseFocusManifest(raw);
        const guidance = buildFocusManifestGuidance({ manifest, changedPaths });
        return guidance.publicNextSteps.every(isFocusManifestPublicSafe);
      }),
    );
  });
});
