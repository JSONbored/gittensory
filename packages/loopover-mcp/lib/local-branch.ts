...
export type McpWorkspaceRoot = { path: string };

export type LocalBranchInput = {
...
export type LocalScorerStatus = {
  ok?: boolean | undefined;
  code?: string | undefined;
  reason?: string | undefined;
  stderr?: unknown;
  scorerCommand?: unknown;
...
type WorkspaceCwdInput = { cwd?: string | null | undefined; workspaceRoots?: unknown };
type ScorePreviewInput = { scorePreviewCommand?: string | undefined };
...
export function collectLocalBranchMetadata(input: LocalBranchInput) {
...
export function buildBranchAnalysisPayload(input: LocalBranchInput) {
  const workspace = resolveWorkspaceCwd(input);
...
export function resolveWorkspaceCwd(input: WorkspaceCwdInput = {}) {
...
export function normalizeMcpWorkspaceRoots(roots: unknown): McpWorkspaceRoot[] {
  if (!Array.isArray(roots)) return [];
  const normalized: McpWorkspaceRoot[] = [];
  const seen = new Set();
...
export function resolveScorePreviewCommand(input: ScorePreviewInput = {}): string | undefined {
  const explicit = input.scorePreviewCommand ?? process.env.GITTENSOR_SCORE_PREVIEW_CMD;
  if (typeof explicit === "string" && explicit.trim()) return explicit.trim();
  return undefined;
}

export function referenceScorePreviewExample(kind: string = "metadata"): string {
...
export function sanitizeLocalScorerStatus(status: LocalScorerStatus): LocalScorerStatus {
...
export function runExternalScorePreview(metadata: Record<string, unknown>, scorerCommand: string | undefined): LocalScorerStatus {
...
export function setupGuidanceForLocalScorer(status: LocalScorerStatus): string[] {
  if (status.ok) return [];
...
export function probeLocalScorer(scorerCommand: string | undefined = resolveScorePreviewCommand()): LocalScorerStatus {
...
function collectChangedFiles(cwd: string, baseRef: string) {
  ...
  const numstat = new Map(parseNumstat(cwd, baseRef).map((entry) => [entry.path, entry] as [string, NumstatEntry]));
...
function normalizeScorerOutput(payload: any) {
  return stripUndefined({
...
function metadataOnlyScorer(status: LocalScorerStatus) {
  return {
    mode: "metadata_only",
...
function scorerFailure(code: string, reason: string, extra: LocalScorerStatus = {}): LocalScorerStatus {
  return stripUndefined({
    ok: false,
    code,
    reason,
...
function classifyScorerExecFailure(error: unknown, durationMs: number, scorerCommand: string): LocalScorerStatus {
  const execError = error && typeof error === "object" ? error as ExecErrorLike : undefined;
...
export function extractLinkedIssues(text: string): number[] {
  const issues: number[] = [];
