...
type WorkspaceRoot = { uri?: unknown };
type NormalizedWorkspaceRoot = { path: string };
type ChangedFile = {
  path: string;
...
type ScorerPayload = Record<string, unknown> & {
  source?: { tokenScore?: unknown; lines?: unknown };
  total?: { tokenScore?: unknown };
  tests?: { tokenScore?: unknown };
  nonCode?: { tokenScore?: unknown };
};
type ExecError = {
  stdout?: unknown;
  stderr?: unknown;
...
export function collectLocalDiff(cwd: string, baseRef: string, workspaceRoots: unknown) {
...
export function collectLocalBranchMetadata(input: BranchInput): BranchMetadata {
...
export function buildBranchAnalysisPayload(input: BranchInput) {
  const workspace = resolveWorkspaceCwd(input);
  ...
  const localScorer = externalPreview.ok ? normalizeScorerOutput(externalPreview.payload as ScorerPayload) : metadataOnlyScorer(externalPreview);
  return {
    ...metadata,
    localScorer,
...
export function resolveWorkspaceCwd(input: Pick<BranchInput, "cwd" | "workspaceRoots"> = {}) {
...
export function normalizeMcpWorkspaceRoots(roots: unknown): NormalizedWorkspaceRoot[] {
  if (!Array.isArray(roots)) return [];
  const normalized: NormalizedWorkspaceRoot[] = [];
...
function safeResolvedPath(path: unknown): string {
...
export function resolveScorePreviewCommand(input: Pick<BranchInput, "scorePreviewCommand"> = {}): string | undefined {
...
export function sanitizeLocalScorerStatus(status: ScorerStatus): ScorerStatus;
export function sanitizeLocalScorerStatus(status: unknown): unknown;
export function sanitizeLocalScorerStatus(status: unknown): unknown {
  if (!status || typeof status !== "object") return status;
  const scorerStatus = status as ScorerStatus;
...
export function runExternalScorePreview(metadata: ScorePreviewMetadata, scorerCommand: string | undefined): ScorerStatus {
  ...
  try {
    ...
    const normalized = normalizeScorerOutput(payload as ScorerPayload);
...
export function probeLocalScorer(scorerCommand: string | undefined = resolveScorePreviewCommand()) {
...
function parseNameStatus(cwd: string, baseRef: string): Array<{ code: string; path: string; previousPath?: string | undefined }> {
  ...
  const entries: Array<{ code: string; path: string; previousPath?: string | undefined }> = [];
...
function parseNumstat(cwd: string, baseRef: string): Array<{ path: string; additions: number; deletions: number; binary: boolean }> {
...
function normalizeScorerOutput(payload: ScorerPayload): ScorerPayload {
  return stripUndefined({
...
function metadataOnlyScorer(status: ScorerStatus): ScorerPayload {
  return {
    mode: "metadata_only",
...
function scorerFailure(code: string, reason: string, extra: ScorerPayload = {}): ScorerStatus {
  return stripUndefined({
    ok: false,
    code,
    reason,
...
function classifyScorerExecFailure(error: unknown, durationMs: number, scorerCommand: string): ScorerStatus {
  const execError = error && typeof error === "object" ? (error as ExecError) : undefined;
...
function inferScorerCode(reason: unknown): string {
...
function titleFromBranch(branchName: string): string {
