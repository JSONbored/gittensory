...
/** The awaited response shape the injected fetch resolves to. */
type ContextFetchResponse = Awaited<ReturnType<SelfReviewContextFetch>>;

/** Resolved ORB session auth for the live-gate probe. */
type ResolvedLoopoverAuth = { apiUrl: string; sessionToken: string };

/** Fully-resolved option set with every default applied. */
type ResolvedContextOptions = {
  ...
  requestTimeoutMs: number;
  liveGateProbeTimeoutMs: number;
  loopoverAuth: ResolvedLoopoverAuth | null;
};

type RepoTarget = { owner: string; repo: string };

/** Minimal GitHub REST issue payload shape this module reads. */
...
/** Minimal GitHub REST pull-request payload shape this module reads. */
...
/** Minimal GitHub REST repository payload shape this module reads. */
...
/** A byte-bounded manifest response — a superset the injected fetch's response satisfies (headers/body optional). */
type StreamReader = {
...
function parseRepoFullName(repoFullName: string): RepoTarget | null {
...
function normalizeOptions(options: FetchSelfReviewContextOptions = {}): ResolvedContextOptions {
  const env = options.env ?? process.env;
  // Explicit null skips the probe (tests / forced-standalone). Undefined ⇒ resolve from loopover-mcp session.
  const loopoverAuth: ResolvedLoopoverAuth | null =
  ...
  return {
    ...
    perPage: Number.isInteger(options.perPage) && (options.perPage as number) > 0 ? (options.perPage as number) : DEFAULT_PER_PAGE,
    maxPages: Number.isInteger(options.maxPages) && (options.maxPages as number) > 0 ? (options.maxPages as number) : DEFAULT_MAX_PAGES,
    ...
    requestTimeoutMs: Number.isInteger(options.requestTimeoutMs) && (options.requestTimeoutMs as number) > 0 ? (options.requestTimeoutMs as number) : DEFAULT_REQUEST_TIMEOUT_MS,
    liveGateProbeTimeoutMs:
      Number.isInteger(options.liveGateProbeTimeoutMs) && (options.liveGateProbeTimeoutMs as number) > 0
        ? (options.liveGateProbeTimeoutMs as number)
...
export function parseLiveGateThresholdFields(payload: unknown): LiveGateThresholdFields | null {
  ...
  const record = payload as { confidence_floor?: unknown; scope_cap_files?: unknown; scope_cap_lines?: unknown };
 ...
 */
export function applyLiveGateThresholdsToManifest(manifest: FocusManifest, fields: LiveGateThresholdFields | null): FocusManifest {
...
async function probeLiveGateThresholds(target: RepoTarget, resolved: ResolvedContextOptions): Promise<LiveGateThresholdFields | null> {
...
async function fetchWithTimeout(
  ...
  timeoutMs: number,
): Promise<ContextFetchResponse> {
  return fetchImpl(url, { ...init, signal: AbortSignal.timeout(timeoutMs) });
}

async function githubGetJson(url: string, resolved: ResolvedContextOptions): Promise<{ response: ContextFetchResponse; payload: unknown }> {
...
async function fetchPaginated(pathWithQuery: string, resolved: ResolvedContextOptions): Promise<unknown[]> {
...
async function fetchRepositoryRecord(target: RepoTarget, resolved: ResolvedContextOptions): Promise<RepositoryRecord | null> {
  ...
  const repo = payload as GithubRepoPayload;
  return {
...
function extractLinkedPrNumbers(body: string): number[] {
...
function extractLinkedIssueNumbers(body: string, repoFullName: string): number[] {
...
async function fetchOpenIssueRecords(target: RepoTarget, resolved: ResolvedContextOptions): Promise<IssueRecord[]> {
  ...
  return payloads
    .filter((issue) => issue && typeof issue === "object" && !(issue as { pull_request?: unknown }).pull_request)
    .map((issue) => toIssueRecord(`${target.owner}/${target.repo}`, issue as GithubIssuePayload));
}

function mergeableBooleanState(mergeable: unknown): "clean" | "dirty" | null {
...
function toPullRequestRecord(repoFullName: string, pr: GithubPrPayload): PullRequestRecord {
  const body = pr.body ?? "";
  return {
    repoFullName,
    number: pr.number,
...
async function fetchOpenPullRequestRecords(target: RepoTarget, resolved: ResolvedContextOptions): Promise<PullRequestRecord[]> {
  const payloads = await fetchPaginated(
    `/repos/${encodeURIComponent(target.owner)}/${encodeURIComponent(target.repo)}/pulls?state=open&sort=created&direction=asc`,
    resolved,
  );
  return payloads.map((pr) => toPullRequestRecord(`${target.owner}/${target.repo}`, pr as GithubPrPayload));
}
...
async function readBoundedManifestResponseText(response: ManifestResponse): Promise<string | null> {
...
async function fetchManifestContent(target: RepoTarget, resolved: ResolvedContextOptions): Promise<string | null> {
...
async function fetchConfirmedContributor(login: string, resolved: ResolvedContextOptions): Promise<boolean> {
...
function computeInDuplicateCluster(collisionReport: ReturnType<typeof buildCollisionReport>, targetIssueNumbers: number[]): boolean {
 ...
 */
export async function fetchSelfReviewContext(repoFullName: string, options: FetchSelfReviewContextOptions = {}): Promise<SelfReviewContextResult> {
