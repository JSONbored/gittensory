...
function normalizeOptions(options: PollCheckRunsOptions = {}): ResolvedPollOptions {
  return {
...
function githubError(response: { status: number }, payload: unknown): Error & { code: string; githubMessage: string | null } {
  const code = `github_${response.status}`;
  const record = payload as { message?: unknown } | null | undefined;
  const githubMessage =
...
async function githubGetJsonResponse(url: string, options: ResolvedPollOptions): Promise<{ payload: unknown; response: Response }> {
...
async function githubGetJson(url: string, options: ResolvedPollOptions): Promise<unknown> {
...
function payloadTotalCount(payload: unknown): number | null {
  const totalCount = Number((payload as { total_count?: unknown } | null | undefined)?.total_count);
...
function normalizeConclusion(checkRun: unknown): CheckRunConclusion {
  ...
  const record = checkRun as { status?: unknown; conclusion?: unknown };
  if (record.status !== "completed") return "pending";
...
function normalizeCheckRun(checkRun: unknown): NormalizedCheckRun {
  const record = checkRun as
    | { name?: unknown; status?: unknown; details_url?: unknown; started_at?: unknown; completed_at?: unknown }
    | null
    | undefined;
  return {
...
function backoffDelayMs(attemptIndex: number, options: ResolvedPollOptions): number {
  const exponent = Math.min(10, Math.max(0, attemptIndex));
  return Math.min(options.maxIntervalMs, options.minIntervalMs * 2 ** exponent);
}

async function fetchHeadSha(target: RepoTarget, prNumber: number, options: ResolvedPollOptions): Promise<string> {
  const payload = await githubGetJson(
    apiUrl(options.apiBaseUrl, repoPath(target, `/pulls/${prNumber}`)),
    options,
  );
  const headSha = (payload as { head?: { sha?: unknown } } | null | undefined)?.head?.sha;
  if (typeof headSha !== "string" || !headSha) throw new Error("github_pr_head_sha_missing");
  return headSha;
}

async function fetchCheckRuns(target: RepoTarget, headSha: string, options: ResolvedPollOptions): Promise<NormalizedCheckRun[]> {
  ...
  while (true) {
    ...
    const checkRuns = (payload as { check_runs?: unknown } | null | undefined)?.check_runs;
