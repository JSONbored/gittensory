...
import { type RepoStackResult, detectRepoStack, renderStackSummary } from "./stack-detection.js";
...
function resolveClaimStatus(claimLedger: CodingTaskClaimLedger, repoFullName: string, issueNumber: number): "claimed" | "unclaimed" {
...
function resolveDuplicateClusterRisk(
  repoFullName: string,
  issues: IssueRecord[],
 ...
 */
function buildValidationGuidance(stack: RepoStackResult): string {
  const lines = [
 ...
 */
function buildInstructions(issue: CodingTaskIssue, acceptanceCriteriaPath: string, stack: RepoStackResult): string {
...
export function buildCodingTaskSpec(input: CodingTaskSpecInput): CodingTaskSpecResult {
  ...
  return {
    ...
    feasibility,
    acceptanceCriteriaPath: writeResult.path as string,
    instructions: buildInstructions(input.issue, writeResult.path as string, stack),
