...
import type { HandoffPacket } from "@loopover/engine";
...
export function evaluateAndRecordHarnessSubmissionTrigger(
  candidate: HarnessSubmissionCandidateInput,
  deps: HarnessSubmissionDeps,
): HarnessSubmissionResult {
 ...
 */
export function prepareOpenPrSubmission(
  candidate: PrepareOpenPrSubmissionCandidate,
  deps: HarnessSubmissionDeps,
): PrepareOpenPrSubmissionResult {
  if (!candidate || typeof candidate !== "object") throw new Error("invalid_harness_submission_candidate");
