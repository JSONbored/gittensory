...
export function recordPrOutcomeSnapshot(input: PrOutcomeInput, options: RecordPrOutcomeOptions = {}): unknown {
  ...
  return eventLedger.appendEvent({ type: MINER_PR_OUTCOME_EVENT, repoFullName, payload: payload as unknown as Record<string, unknown> });
}
...
export function readPrOutcomes(
  ...
  for (const rawEvent of Array.isArray(events) ? events : []) {
    const event = rawEvent as { type?: unknown; repoFullName?: unknown; payload?: unknown } | null | undefined;
