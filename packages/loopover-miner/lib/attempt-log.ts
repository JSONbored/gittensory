...
import { type AttemptLogEvent, type NormalizedAttemptLogEvent, formatAttemptLogJsonl, normalizeAttemptLogEvent } from "@loopover/engine";
...
/** SQLite `attempt_log_events` row shape (StatementSync returns `unknown`). */
type AttemptLogRow = {
  id: number;
  seq: number;
  attempt_id: string;
...
function rowToNormalized(row: AttemptLogRow): NormalizedAttemptLogEvent {
  return {
    eventType: row.event_type as NormalizedAttemptLogEvent["eventType"],
    attemptId: row.attempt_id,
    actionClass: row.action_class,
    mode: row.mode as NormalizedAttemptLogEvent["mode"],
    reason: row.reason,
...
function ensureOutcomeColumns(db: DatabaseSync): void {
  const existingColumns = new Set(
    db.prepare("PRAGMA table_info(attempt_log_events)").all().map((column) => (column as { name: string }).name),
  );
...
export function initAttemptLog(dbPath: string = resolveAttemptLogDbPath()): AttemptLog {
  ...
  return {
    ...
    appendAttemptLogEvent(event) {
      ...
      try {
        const { nextSeq } = nextSeqStatement.get() as { nextSeq: number };
