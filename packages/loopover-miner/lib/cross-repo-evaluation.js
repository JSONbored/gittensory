...
export async function evaluateRepoFullExecution(entry, options = {}) {
    ...
    if (buildCommand && typeof options.buildRepo === "function") {
        ...
        if (!built.ok) {
            return buildExecutionFailure(repoFullName, CROSS_REPO_EXECUTION_CATEGORY.CODE_BUILD_FAILED, `Build failed: ${built.detail ?? buildCommand}`, { readinessPassed: true, diffPresent, built: false, stack });
        }
    }
    ...
    if (typeof runRepoTests !== "function") {
        return buildExecutionFailure(repoFullName, CROSS_REPO_EXECUTION_CATEGORY.OTHER, "No test runner was provided; full execution cannot run the repo's tests.", { readinessPassed: true, diffPresent, built: true, stack });
    }
    const tested = await runRepoTests({ repoPath, command: testCommand });
    if (!tested.ok) {
        return buildExecutionFailure(repoFullName, CROSS_REPO_EXECUTION_CATEGORY.TESTS_FAILED, `Tests failed: ${tested.detail ?? testCommand}`, { readinessPassed: true, diffPresent, built: true, testsPassed: false, stack });
    }
    // Tests passed -- but an empty diff means the agent changed nothing, so the pass is trivial (no real code).
    if (!diffPresent) {
        return buildExecutionFailure(repoFullName, CROSS_REPO_EXECUTION_CATEGORY.NO_OP_DIFF, "Tests passed but the agent produced an empty diff (no real change).", { readinessPassed: true, diffPresent: false, built: true, testsPassed: true, stack });
    }
    return {
        repoFullName,
        passed: true,
