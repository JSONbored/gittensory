...
/** The core fields a detector contributes; `detected: true` is stamped on by `detectRepoStack`. */
type DetectedStackCore = Omit<DetectedRepoStack, "detected">;

/** A never-throwing accessor over the cloned repo. */
type RepoAccess = {
...
export const RECOGNIZED_MANIFESTS: readonly string[] = Object.freeze([
  "package.json",
  "pyproject.toml",
  "setup.py",
  "setup.cfg",
...
const NODE_PACKAGE_MANAGERS: readonly string[] = Object.freeze(["npm", "yarn", "pnpm", "bun"]);
...
function makeAccess(repoPath: string, options: DetectRepoStackOptions): RepoAccess {
  const existsImpl: (path: string) => boolean = options.existsSync ?? existsSync;
  const readImpl: (path: string, encoding: "utf8") => string = options.readFileSync ?? readFileSync;
...
function pickScript(scripts: Record<string, unknown>, exactName: string, pattern: RegExp): string | null {
...
function nodeLockfile(exists: (relativePath: string) => boolean): string | null {
  const match = NODE_LOCKFILES.find(([file]) => exists(file));
  return match ? match[0] : null;
}

function nodePackageManager(pkg: Record<string, unknown> | null, lockfile: string | null): string {
...
function hasTypescriptDependency(pkg: Record<string, unknown> | null): boolean {
...
function detectNode({ exists, read }: RepoAccess): DetectedStackCore | null {
  if (!exists("package.json")) return null;
  const pkg = parseJson(read("package.json"));
  const scripts: Record<string, unknown> =
    pkg && typeof pkg.scripts === "object" && pkg.scripts && !Array.isArray(pkg.scripts)
...
function detectPython({ exists, read }: RepoAccess): DetectedStackCore | null {
...
function detectRust({ exists }: RepoAccess): DetectedStackCore | null {
...
function detectGo({ exists }: RepoAccess): DetectedStackCore | null {
...
function detectMaven({ exists }: RepoAccess): DetectedStackCore | null {
...
function detectGradle({ exists }: RepoAccess): DetectedStackCore | null {
...
const DETECTORS: readonly ((access: RepoAccess) => DetectedStackCore | null)[] = Object.freeze([
  detectNode,
  detectPython,
  detectRust,
  detectGo,
