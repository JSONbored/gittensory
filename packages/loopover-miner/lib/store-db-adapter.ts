...
class Statement implements MinerD1PreparedStatement {
  driver: SqliteDriver;
  sql: string;
  values: unknown[];
  ...
  execSync(): { results: Record<string, unknown>[]; success: true; meta: Record<string, unknown> } {
  ...
  async all<T = unknown>(): Promise<{ results: T[]; success: true; meta: Record<string, unknown> }> {
    return this.execSync() as unknown as { results: T[]; success: true; meta: Record<string, unknown> };
  }
  ...
  async raw<T = unknown>(): Promise<T[]> {
    return this.driver.query(this.sql, this.values).rows.map((row) => Object.values(row)) as unknown as T[];
  }
}
...
export function createD1Adapter(driver: SqliteDriver): MinerD1Database {
  return {
    ...
    async batch(statements: MinerD1PreparedStatement[]) {
      driver.exec("BEGIN");
      try {
        const out = statements.map((s) => (s as Statement).execSync());
        driver.exec("COMMIT");
        return out;
      } catch (error) {
        try {
