...
/** Internal validated/normalized candidate shape ready for insertion (all dimensions defaulted). */
...
function normalizeCandidate(candidate: unknown): NormalizedRankedCandidate {
  ...
  if (!Number.isInteger(issueNumber) || (issueNumber as number) <= 0) throw new Error("invalid_ranked_candidate");
