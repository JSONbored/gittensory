...
export type GateCheckPolicy = {
  ...
  /** Linked-issue satisfaction gate (#1961/#3906, #4518 engine-parity port). When `block`, a
   *  `linked_issue_scope_mismatch` finding — raised when the AI assessment judged (above its confidence
   *  floor) that the PR's diff does NOT satisfy its primary linked issue's intent — becomes a hard blocker.
   *  Defaults to `advisory` — the finding is never even produced under `advisory`/`off` (the caller gates the
   *  assessment itself on this mode), so this branch only matters once a repo has explicitly opted into `block`. */
...
function isConfiguredGateBlocker(finding: AdvisoryFinding, policy: GateCheckPolicy): boolean {
  ...
  // finding itself is only ever produced when the caller already resolved `block` mode, so this is a
  // defense-in-depth mirror of that gate, not the primary enforcement point.
