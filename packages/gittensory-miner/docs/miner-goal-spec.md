...
back to its default with a warning, so a broken file never hard-fails a miner. The JSON Schema mirrors this: it sets
`additionalProperties: true`, so an unknown key validates cleanly rather than failing, matching the parser.
