...
//
// Extracted out of `src/signals/redaction.ts` into the shared engine (#4883): it is confirmed-pure,
// dependency-free string logic (no D1, no Env, no network) with high fan-in across public surfaces, so the
// canonical vocabulary now lives in the shared engine and `src/signals/redaction.ts` is a re-export shim.
// Existing consumers' import paths are unchanged; behavior is byte-for-byte identical.
//
